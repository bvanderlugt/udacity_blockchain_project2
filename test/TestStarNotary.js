...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(name, "Crypto Starz");
    assert.equal(symbol, "STARZ");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(user1, ownerOfStar1);
    assert.equal(user2, ownerOfStar2);
    await instance.exchangeStars(starId1, starId2, {from: user1});
    ownerOfStar1 = await instance.ownerOf(starId1);
    ownerOfStar2 = await instance.ownerOf(starId2);
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('rad star', starId, {from: user1});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    let starId = 11;
    await instance.createStar('rad star', starId, {from: accounts[0]});
    let info = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(info, 'rad star');
});